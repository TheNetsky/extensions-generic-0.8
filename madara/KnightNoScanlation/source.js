...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var KnightNoScanlationParser = class extends Parser3 {
    async parseMangaDetails($2, mangaId, source) {
      const image = encodeURI(await this.getImageSrc($2("div.summary_image img").first(), source));
      const titles = [];
      titles.push((0, import_html_entities2.decode)($2("div.post-title h1, div#manga-title h1").children().remove().end().text().trim()));
      let parsedStatus = "";
      let description = "";
      for (const obj of $2("div.post-content_item").toArray()) {
        switch ((0, import_html_entities2.decode)($2("h5", obj).first().text()).trim().toUpperCase()) {
          case "ALTERNATIVE":
            titles.push((0, import_html_entities2.decode)($2("div.summary-content", obj).text().trim()));
            break;
          case "STATUS":
            parsedStatus = $2("div.summary-content", obj).text().trim();
            break;
          case "SUMMARY":
            description = (0, import_html_entities2.decode)($2("p", obj).text().trim());
            break;
        }
      }
      let status;
  ...
  // src/KnightNoScanlation/KnightNoScanlation.ts
  var DOMAIN = "https://lectorkns.eyudud.net";
