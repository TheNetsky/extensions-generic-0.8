...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  var SkyMangasParser = class extends MangaStreamParser {
    parseChapterDetails($2, mangaId, chapterId) {
      const pages = [];
      let obj;
      for (const script of $2('script[src*="data:text/javascript"]').toArray()) {
      ...
      for (const index2 of obj.sources) {
        if (index2?.images.length == 0) continue;
        index2.images.map((p) => pages.push(encodeURI(p)));
      }
      const chapterDetails = App.createChapterDetails({
        id: chapterId,
        mangaId,
  ...
  var SkyMangas = class extends MangaStream {
    ...
    configureSections() {
      this.homescreen_sections["popular_today"].selectorFunc = ($2) => $2("div.bsx", $2("h2:contains(Popular Today)")?.parent()?.next());
