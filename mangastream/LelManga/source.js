...
var _Sources = (() => {
  ...
  // src/LelManga/LelManga.ts
  var LelManga_exports = {};
  __export(LelManga_exports, {
    LelManga: () => LelManga,
    LelMangaInfo: () => LelMangaInfo
  });
  ...
  var MangaStreamParser = class {
    ...
    parseMangaDetails($, mangaId, source) {
      const titles = [];
      titles.push((0, import_html_entities.decode)($("h1.entry-title").text().trim()));
      ...
      const description = (0, import_html_entities.decode)($('div[itemprop="description"]  p').text().trim());
      const arrayTags = [];
    ...
    parseChapterList($, mangaId, source) {
      ...
      for (const chapter of $("li", "div#chapterlist").toArray()) {
        const title = (0, import_html_entities.decode)($("span.chapternum", chapter).text().trim());
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`);
  ...
  // src/LelManga/LelManga.ts
  var DOMAIN = "https://www.lelmanga.com";
  var LelMangaInfo = {
  ...
  var LelManga = class extends MangaStream {
    constructor() {
      super(...arguments);
      this.baseUrl = DOMAIN;
    ...
    configureSections() {
      ...
      this.homescreen_sections["latest_update"].selectorFunc = ($) => $("div.uta", $("h2:contains(Derni\xE8res Sorties)")?.parent()?.next());
      this.homescreen_sections["new_titles"].enabled = false;
    }
  };
  return __toCommonJS(LelManga_exports);
})();
